...
def start():
  ...
  # on = runsen()
   ...
   templateData = {
      'title': 'SDS011 Data',
      'time': timeString,
      'pm2': sdsdata.iloc[~0]["sds-pm2.5"],
      'pm10': sdsdata.iloc[~0]["sds-pm10"],
      'Maxpm10': max(sdsdata["sds-pm10"]),
      'Maxpm2': max(sdsdata["sds-pm2.5"]),
           'Minpm2': min(sdsdata["sds-pm2.5"]),
		      'Minpm10': min(sdsdata["sds-pm10"]),   
...
def plot_pm():
  ...
  axis.set_title("SDS011")
  axis.grid(True)
  axis.plot(sdsdata["sds-pm2.5"],label="PM2.5")
  axis.plot(sdsdata["sds-pm10"],label="PM10")
  axis.set_xlim(sdsdata.index[0],sdsdata.index[~0])
  axis.legend()
